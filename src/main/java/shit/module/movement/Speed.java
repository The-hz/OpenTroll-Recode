...
extends Module {
    ...
    private final NumberSetting boost = (NumberSetting)this.m28(new NumberSetting("Boost", 0.0, 0.0, 2.0, 0.05));
    ...
    private void setEvent2Inner15(Event2.Event2Inner event2Inner) {
        ...
        double d = ((Double)this.speed.getObj()).doubleValue();
        Mode mode = (Mode)((Object)this.mode.getObj());
        switch (mode) {
            case STRAFE: {
                if (!MC.client3.player.isOnGround()) break;
                if (!MathUtil.isSet7()) break;
                double[] dArray = MathUtil.m246(d);
                MC.client3.player.setVelocity(dArray[0], MC.client3.player.getVelocity().y, dArray[1]);
                break;
            }
            case ON_GROUND: {
                if (!MC.client3.player.isOnGround()) break;
                if (!MathUtil.isSet7()) break;
                double[] dArray = MathUtil.m246(d);
                MC.client3.player.setVelocity(dArray[0], 0.0, dArray[1]);
            ...
            case VANILLA: {
                ...
                if (!MathUtil.isSet7()) break;
                double[] dArray = MathUtil.m246(d + ((Double)this.boost.getObj()).doubleValue());
                MC.client3.player.setVelocity(dArray[0], MC.client3.player.getVelocity().y, dArray[1]);
                break;
            }
        }
    ...
    private void setPacketEventInner25(PacketEvent.PacketEventInner2 packetEventInner2) {
        ...
        if (this.mode.getObj() != Mode.ON_GROUND) {
            return;
        }
